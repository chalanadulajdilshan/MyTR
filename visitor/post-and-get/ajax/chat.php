...
if ($_POST['action'] == 'LOGIN_SEND_MESSAGE') {
    $allparticipants = VisitorMessage::getDistinctCustomer($_POST["visitor"]);
    $VISITOR_MESSAGE = new VisitorMessage(NULL);


    $VISITOR_MESSAGE->message = 'Wel Come to MytravelPartner in Sri lanka.';
...
if ($_POST['action'] == 'GETMESSAGES') {
    ...
    $final_arr['image_name_visitor'] = $CUSTOMER->image_name;
    $final_arr['visitor_name'] = $CUSTOMER->full_name;
    $final_arr['is_type'] = $CUSTOMER->is_type;
    $final_arr['id'] = $CUSTOMER->id;
    if (Helper::checkIsOnline($CUSTOMER->lastLogin)) {
...
if ($_POST['action'] == 'UPDATE_TYPING') {
    ...
    $VISITOR->is_type = $_POST['is_type'];
    $result = $VISITOR->updatIsTypeing();
