$(document).ready(function () {
    $("#send-sms-code").click(function (event) {
        window.swal({
            title: "Please wait...!",
        ...
        var id = $("#id").val();
        $.ajax({
            url: "post-and-get/ajax/mobile-verify.php",
            type: "POST",
            data: {id: id, action: "MOBILECODE"},
            dataType: "JSON",
            success: function (result) {
                if (result.status == 'success') {
                    setTimeout(function () {
                        window.location.href = "verification-mobile.php";
                    }, 1000);
                } else {
                    swal({
                        title: "Error!",
    ...
    $("#verify").click(function (event) {
        ...
        if (!$('#code').val() || $('#code').val().length === 0) {
            ...
            $.ajax({
                url: "post-and-get/ajax/mobile-verify.php",
                type: "POST",
                data: {id: id, code: code, action: "MOBILEVERYFY"},
                dataType: "JSON",
                success: function (result) {
                    if (result.status == 'success') {
                        window.swal({
    ...
    setTimeout(
            function ()
            {
                ...
                $('#send_phone_verification').css('cursor', 'pointer'); 
                $('#countdown_p').hide();
            }, 121000);
